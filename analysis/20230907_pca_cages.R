...
# Plot the first two principal components with PVE
ggplot(pca_df, aes(x = PC1, y = PC2, color = group)) +
  geom_point(size = 3, alpha = 0.7) +
  theme_minimal() +
  labs(title = "PCA of Allele Frequencies -- Grouped by Cage", x = paste0("PC1 (", pve[1], "%)"), y = paste0("PC2 (", pve[2], "%)")) +
  theme(legend.position = "bottom") + 
  geom_label(aes(label = group), nudge_x = 0.1, nudge_y = 0.1, size = 3)

ggsave("/Genomics/ayroleslab2/scott/git/chromium/analysis/figures/pca_af_cages.jpeg", width = 10, height = 10, units = "in")
