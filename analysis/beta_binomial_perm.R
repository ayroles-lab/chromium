...
colnames(output_mat) <- c("obs_ctrl","pval_ctrl","obs_crvi","pval_crvi","obs_contrast","pval_contrast","fdr_pval","significant")
tic()
for (i in 1:number_of_loci){
    ...
    mod2 <- tryCatch(betabin(cbind(alt_depth, depth - alt_depth) ~ treatment + sex, ~1,data=dt_no_t0), error=function(x){})
    ...
    dt_no_t0[, .(fitted = f(betabin(cbind(alt_depth, depth - alt_depth) ~ treatment + sex , random = ~1, data = .SD)))]
...
write.table(output_mat, file=paste0("/Genomics/ayroleslab2/scott/git/chromium/data/betabin_perm_output_10perm_fdr05_",file_number,".csv"), sep=",", row.names=FALSE, col.names=TRUE)
